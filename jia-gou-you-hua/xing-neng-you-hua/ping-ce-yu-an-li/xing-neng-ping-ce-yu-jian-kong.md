...
白屏时间；首屏时间；用户可交互时间；总下载时间； DNS 解析时间； TCP 连接时间； HTTP 请求时间； HTTP 响应时间；
...
然后在 HRTime 之上，是一个叫 PerformanceTimeline 的 API，这个 API 很简单，它只是去定义了把各种各样的性能的情况。 在此，我们得出的最佳实践是：采集性能数据时先抹平 Navigation Timing spec 差异，优先使用 PerformanceTimeline API\(在复杂场景，亦可考虑优先使用 PerformanceObserver\)。
...
有一个点:mod\_36ad799.js 等几个 js 为什么会在 hm.js 之前下载？html 代码如下:
...
* [https://medium.freecodecamp.org/creating-a-web-performance-culture-inside-your-team-f00c0d79765f](https://medium.freecodecamp.org/creating-a-web-performance-culture-inside-your-team-f00c0d79765f)
